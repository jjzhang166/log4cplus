...
namespace log4cplus {
    namespace helpers {
         ...
         */
        LOG4CPLUS_EXPORT std::string toUpper(const std::string& s);
         ...
         */
        LOG4CPLUS_EXPORT std::string toLower(const std::string& s);
        ...
        inline std::string convertIntegerToString(intType intValue)
        {
            std::string result;
        ...
        inline void join(std::string& result, Iterator start, Iterator last, std::string const& sep)
        {
            if(start != last)
                result = *start++;
