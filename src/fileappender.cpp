...
using namespace helpers;
	...
	namespace
	{
		...
		static long file_rename(string const& src, string const& target)
		{
		...
		static long file_remove(string const& src)
		{
		...
		static void loglog_renaming_result(helpers::LogLog & loglog, string const& src, string const& target, long ret)
		{
			if(ret == 0)
			{
		...
		static void loglog_opening_result(helpers::LogLog & loglog, ostream const& os, string const& filename)
		{
			if(!os)
			{
	...
	FileAppender::FileAppender(const helpers::Properties& props, std::ios_base::openmode mode_)
	...
	RollingFileAppender::RollingFileAppender(const helpers::Properties& properties)
		: FileAppender(properties, std::ios_base::app)
	{
		long tmpMaxFileSize = DEFAULT_ROLLING_LOG_SIZE;
		int tmpMaxBackupIndex = 1;
		string tmp(helpers::toUpper(properties.getProperty("MaxFileSize")));
		if(!tmp.empty())
		{
	...
	DailyRollingFileAppender::DailyRollingFileAppender(const helpers::Properties& properties)
		: FileAppender(properties, std::ios_base::app), _maxBackupIndex(10)
	{
		DailyRollingFileSchedule theSchedule = DAILY;
		string scheduleStr(helpers::toUpper(
